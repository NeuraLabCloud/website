...
import { api } from "../../../convex/_generated/api" // The database schema types
...
export function ConvexAdapter(): Adapter {
  return {
    ...
    async getUserByAccount({ providerAccountId, provider }) {
      try {
        const user = await convexClient.query(api.account.getUserByAccount, {
            providerAccountId,
        })
    ...
    async linkAccount(account) {
     try {
        await convexClient.mutation(api.account.create, {
    ...
    async unlinkAccount({ providerAccountId, provider }) {
      await convexClient.mutation(api.account.deleteAccount, {
        providerAccountId
      })
    },
